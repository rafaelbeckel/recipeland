    ...
    public function run()
    {
        ...
        for ($count = 0; $count < getenv('RECIPE_SEED_COUNT'); ++$count) {
            ...
            foreach ($steps as $step_name) {
                ...
                // Steps need to be coherent with Recipe's title
                if ($step_name == $prepare_actions[0]) {
                    $description = ucfirst($step_name).' '.$spices[0];
                } elseif ($step_name == $cook_actions[0]) {
                    $description = ucfirst($step_name).' '.$ingredients[0];
                } else {
